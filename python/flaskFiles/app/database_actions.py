...
class database_actions:
    ...
    def seed():
        ...
        project2 = projects(id=2,project_name="testproject2")
        db.session.add(project2)
        #test runs
        testrun1 = testRun(id=1,name="testrun1",project=1,date=None)
        db.session.add(testrun1)
        testrun2 = testRun(id=2,name="testrun2",project=1,date=None)
        db.session.add(testrun2)
        testrun3 = testRun(id=3,name="testrun3",project=2,date=None)
        db.session.add(testrun3)
        testrun4 = testRun(id=4,name="testrun4",project=2,date=None)
        db.session.add(testrun4)
        #test suites
        testsuite1 = test_suite(id=1,testsuite="testsuite1", project=1,runtime=1.1)
        db.session.add(testsuite1)
        testsuite2 = test_suite(id=2,testsuite="testsuite2", project=2,runtime=7.7)
        db.session.add(testsuite2)
        testsuite3 = test_suite(id=3,testsuite="testsuite3", project=2,runtime=1.3)
        db.session.add(testsuite3)
        testsuite4 = test_suite(id=4,testsuite="testsuite4", project=1,runtime=4.35)
        db.session.add(testsuite4)
        #test names
        test1 = test_names(id=1,test_name="test1",project=1)
        db.session.add(test1)
        test2 = test_names(id=2,test_name="test1",project=1)
        db.session.add(test2)
        test3 = test_names(id=3,test_name="test1",project=1)
